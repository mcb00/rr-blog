...
### Decision via prediction failure modes

To help build some intuition for why optimization based on predictive models can fail, let's work with a simple, concrete example.

Suppose in an effort to increase retention, we're deciding whether to give promotional offers to customers.
We have some historical customer data that tells us whether each customer was given an offer and whether they were retained.
...
# 1. Simulate risk (40% high risk, 60% low risk)
risk = np.random.binomial(1, 0.4, n)

# 2. Simulate promo based on risk (high risk much more likely to get promo)
promo_prob = np.where(risk == 1, 0.8, 0.2)
promo = np.random.binomial(1, promo_prob)

# 3. Simulate retention (high risk less likely to retain, promo helps slightly)
retention_prob = np.select(
    [
        (risk == 0) & (promo == 0),
        (risk == 0) & (promo == 1),
        (risk == 1) & (promo == 0),
...
```{python}
df.groupby('promo')['retention'].mean()
```


Wow, offers are associated with lower retention in the historical data.
Our promos must be pretty terrible or something. 

Representing our summary table as a simple model we have

$$ f(x) = E[Y|X=x] $$ 

where $Y$ is the retention status and $X$ is the offer status.
This model implies that 

$$ \underset{x}{\operatorname{argmax}} f(x) = \text{no offer} $$

Plugging our predictive model into Our decision  optimization framework, it looks like the best action is to give no promotional offers. 
Case closed, right?


### Simpson's Paradox

But what if it turns out that the sales team, who gave out those promos in our historical data, was targeting customers who they thought were at risk of churning?
Suppose we were able to recover the logs of which customers they thought were at risk and then we split customers by risk and promo status.

TODO table with risk and promo status

A few very interesting things pop out in this view.
First, we can see that the sales team did in fact tend to target at risk customers with promos.
Second, we see that retention is actually higher for promo customers in both the risk and non risk customer groups, indicating that promos being truly effective.
This situation, where the direction of an association flips after further splitting the data is called Simpson's Paradox.
...
When we talk about choosing an action that yields the best outcome, we are talking about  *causality*. 
We suppose that the system is one in which our actions can influence the outcome.

Don't be shy, let's all say it together:

**Correlation does not necessarily imply causation**.

ML models trained on observational data are basically fancy correlation machines.
They can learn complicated association structures among variables, 
